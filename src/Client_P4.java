...
public class Client_P4 {
	...
	static String location = home + "/ClientP4";
	...
	public static void main(String[] args) {
		Client_P4 cli4 = new Client_P4();
		new File(location + "/chunks/").mkdirs();
		String client_config = null;
		int files_to_recv;
		
		try {
			cli4.read_port_values();
			cli4.client_connect(cli4.main_server_port,cli4.temp);
			client_config = (String) cli4.in_stream.readObject();
			files_to_recv = (int) cli4.in_stream.readObject();
			cli4.chunk_list = Collections.synchronizedSet(new LinkedHashSet<Integer>());
			
			for (int i = 1; i <= files_to_recv; i++)
				cli4.chunk_list.add(i);
			int files_receive = (int) cli4.in_stream.readObject();
			
			while (files_receive > 0) {
				Chunk_File r_chunk_obj = cli4.receive_chunk(cli4.temp);
				if (r_chunk_obj == null)
					System.out.println("chunk received is null");
				else
					cli4.create_chunkfile(chunks_location, r_chunk_obj,cli4.temp);
				files_receive--;
			}
			
			cli4.client_disconnect(cli4.temp);
		...
		Thread thread =new Thread(new Runnable(){
			public void run(){
				cli4.client_server_connect(cli4.client_server_port,cli4.temp);	
		}});
		thread.start();

		cli4.client_connect(cli4.client_neighbour_port,cli4.temp);
		System.out.println("client4 files_to_recv:"+files_to_recv);
		while(true)
		{
			System.out.println("client4 files to download:"+cli4.chunk_list);	
			if(!cli4.chunk_list.isEmpty())
			{
				Integer[] a = cli4.chunk_list.toArray(new Integer[cli4.chunk_list.size()]);
				for(int i=0;i<a.length;i++)
				{
					cli4.out_stream.writeObject(a[i]);
					cli4.out_stream.flush();
					Chunk_File r_chunk_obj = cli4.receive_chunk(cli4.temp);
				if (r_chunk_obj != null)
					cli4.create_chunkfile(chunks_location, r_chunk_obj,cli4.temp);
				}
	...
	public void client_connect(int port,int val) throws InterruptedException {
		...
		try {
			...
			System.out.println(" client4 connected to :" + client_socket);
	...
	public void client_disconnect(int val) {
		try {
			in_stream.close();
			client_socket.close();
			System.out.println("client4 connection closed:"+client_socket);
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
