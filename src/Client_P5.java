...
public class Client_P5 {
	...
	static String location = home + "/ClientP5";
	...
	public static void main(String[] args) {
		Client_P5 cli5 = new Client_P5();
		new File(location + "/chunks/").mkdirs();
		String client_config = null;
		int files_to_recv;
		
		try {
			cli5.read_port_values();
			cli5.client_connect(cli5.main_server_port,cli5.temp);
			client_config = (String) cli5.in_stream.readObject();
			files_to_recv = (int) cli5.in_stream.readObject();
			cli5.chunk_list = Collections.synchronizedSet(new LinkedHashSet<Integer>());
			
			for (int i = 1; i <= files_to_recv; i++)
				cli5.chunk_list.add(i);

			int files_receive = (int) cli5.in_stream.readObject();
			while (files_receive > 0) {
				Chunk_File r_chunk_obj = cli5.receive_chunk(cli5.temp);
				if (r_chunk_obj == null)
					System.out.println("chunk received is null");
				else
					cli5.create_chunkfile(chunks_location, r_chunk_obj,cli5.temp);
				files_receive--;
			}

			cli5.client_disconnect(cli5.temp);
		...
		Thread thread =new Thread(new Runnable(){
			public void run(){
				cli5.client_server_connect(cli5.client_server_port,cli5.temp);
			
		}});
		thread.start();

		cli5.client_connect(cli5.client_neighbour_port,cli5.temp);
		System.out.println("client5 files_to_recv:"+files_to_recv);
		while(true)
		{
			System.out.println("client5 files to download:"+cli5.chunk_list);	
			if(!cli5.chunk_list.isEmpty())
			{
				Integer[] a = cli5.chunk_list.toArray(new Integer[cli5.chunk_list.size()]);
				for(int i=0;i<a.length;i++){
					cli5.out_stream.writeObject(a[i]);
					cli5.out_stream.flush();
					Chunk_File r_chunk_obj = cli5.receive_chunk(cli5.temp);
					if (r_chunk_obj != null)
						cli5.create_chunkfile(chunks_location, r_chunk_obj,cli5.temp);
					}
	...
	public void client_connect(int port,int val) throws InterruptedException {
		...
		try {
			...
			System.out.println(" client5 connected to :" + client_socket);
	...
	public void client_disconnect(int val) {
		try {
			in_stream.close();
			client_socket.close();
			System.out.println("client5 connection closed:"+client_socket);
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
