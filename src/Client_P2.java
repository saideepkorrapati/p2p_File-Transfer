...
public class Client_P2 {
	...
	static String location = home + "/ClientP2";
	...
	public static void main(String[] args) {
		...
		Client_P2 cli2 = new Client_P2();
		new File(location + "/chunks/").mkdirs();	
		String client_config = null;
		int files_to_recv;

		try {
			cli2.read_port_values();
			cli2.client_connect(cli2.main_server_port,cli2.temp);
			client_config = (String) cli2.in_stream.readObject();
			files_to_recv = (int) cli2.in_stream.readObject();
			cli2.chunk_list = Collections.synchronizedSet(new LinkedHashSet<Integer>());
			
			for (int i = 1; i <= files_to_recv; i++)
				cli2.chunk_list.add(i); 

			int files_receive = (int) cli2.in_stream.readObject();
			
			while (files_receive > 0) {
				Chunk_File r_chunk_obj = cli2.receive_chunk(cli2.temp);
				if (r_chunk_obj == null)
					System.out.println("chunk received is null");
				else
					cli2.create_chunkfile(chunks_location, r_chunk_obj,cli2.temp);
				files_receive--;
			}
			cli2.client_disconnect(cli2.temp);
		...
		Thread thread =new Thread(new Runnable(){
			public void run(){
				cli2.client_server_connect(cli2.client_server_port,cli2.temp);
			
		}});
		thread.start();

		cli2.client_connect(cli2.client_neighbour_port,cli2.temp); 
		System.out.println("client2 files_to_recv:"+files_to_recv);
		
		while(true)
		{
			System.out.println("client2 files to download:"+cli2.chunk_list);	
			if(!cli2.chunk_list.isEmpty()){
				Integer[] a = cli2.chunk_list.toArray(new Integer[cli2.chunk_list.size()]);
				for(int i=0;i<a.length;i++)		{
					cli2.out_stream.writeObject(a[i]);
					cli2.out_stream.flush();
					Chunk_File r_chunk_obj = cli2.receive_chunk(cli2.temp);
					if (r_chunk_obj != null)
						cli2.create_chunkfile(chunks_location, r_chunk_obj,cli2.temp);
				}
	...
	public void client_server_connect( int port,int val) {
		try {
			...
			System.out.println(this.getClass().getName()+"Client2-Server socket created, accepting connections...");
			while (true) {
	...
	public void client_connect(int port,int val) throws InterruptedException {
		...
		try {
			...
			System.out.println(" client2 connected to :" + client_socket);
	...
	public void client_disconnect(int val) {
		try {
			in_stream.close();
			client_socket.close();
			System.out.println("client2 connection closed:"+client_socket);
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
