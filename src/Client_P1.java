...
public class Client_P1 {
	...
	static String location = home + "/ClientP1";
	...
	public static void main(String[] args) {
		...
		Client_P1 cli1 = new Client_P1();	
		new File(location + "/chunks/").mkdirs();
		String client_config = null;
		int files_to_recv;

		try {
			cli1.read_port_values();
			cli1.client_connect(cli1.main_server_port,cli1.temp);
			client_config = (String) cli1.in_stream.readObject();
			files_to_recv = (int) cli1.in_stream.readObject();
			cli1.chunk_list = Collections.synchronizedSet(new LinkedHashSet<Integer>());
			
			for (int i = 1; i <= files_to_recv; i++)
				cli1.chunk_list.add(i); 
			int files_receive = (int) cli1.in_stream.readObject();
			
			while (files_receive > 0) {
				Chunk_File r_chunk_obj = cli1.receive_chunk(cli1.temp);
				if (r_chunk_obj == null)
					System.out.println("chunk received is null");
				else
					cli1.create_chunkfile(chunks_location, r_chunk_obj,cli1.temp);
				files_receive--;
			}
			cli1.client_disconnect(cli1.temp);
		...
		Thread thread =new Thread(new Runnable(){
			public void run(){
				cli1.client_server_connect(cli1.client_server_port,cli1.temp);
	
		}});
		thread.start();

		cli1.client_connect(cli1.client_neighbour_port,cli1.temp); 
		System.out.println("client1 files_to_recv:"+files_to_recv);
		
		while(true)
		{
			System.out.println("client1 files to download:"+cli1.chunk_list);		
			if(!cli1.chunk_list.isEmpty()){
				Integer[] a = cli1.chunk_list.toArray(new Integer[cli1.chunk_list.size()]);
				for(int i=0;i<a.length;i++){
					cli1.out_stream.writeObject(a[i]);
					cli1.out_stream.flush();
					Chunk_File r_chunk_obj = cli1.receive_chunk(cli1.temp);
					if (r_chunk_obj != null)
						cli1.create_chunkfile(chunks_location, r_chunk_obj,cli1.temp);
				}
	...
	public void client_connect(int port,int val) throws InterruptedException {
		...
		try {
			...
			System.out.println(" client1 connected to :" + client_socket);
	...
	public void client_disconnect(int val) {
		try {
			in_stream.close();
			client_socket.close();
			System.out.println("client1 connection closed:"+client_socket);
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
